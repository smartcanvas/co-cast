...
(function() {
    ...
    $(document).on('firebase-changed-mock', function(){      
    ...
    var APICaller = {
        ...
        get: function (uri, onSuccess, onError, timeout) {
            ...
            //mock.contents = mock.contents.splice(1,1);
            if (onSuccess) {
              onSuccess(mock);
            }
            /*/
    ...
    CoCastInitializer = {
        ...
        authenticate: function (authSuccess, authError, timeout) {
            ...
            /* mock
              if (authSuccess) {
                CoCastInitializer.accessToken = "mock";
                authSuccess();
              }
            /*/
            $.ajax({
    ...
    window.addEventListener('WebComponentsReady', function() {
        ...
        var networkMnemonic = window.Util.getParameterByName('network');
        var stationMnemonic = window.Util.getParameterByName('station');
        var clientId = window.Util.getParameterByName('clientId');
        var liveToken = window.Util.getParameterByName('liveToken');
